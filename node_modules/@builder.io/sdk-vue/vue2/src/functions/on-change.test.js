...
var import_on_change = __toModule(require("./on-change"));
test("onChange can observe a shallow change", () => {
  let changeHappend = false;
  const obj = (0, import_on_change.onChange)({ foo: "hi" }, () => changeHappend = true);
  obj.foo = "yo";
  expect(changeHappend).toBe(true);
});
test("onChange can observe a deep change", () => {
  let changeHappend = false;
  const obj = (0, import_on_change.onChange)({ foo: { bar: "hi" } }, () => changeHappend = true);
  obj.foo.bar = "yo";
  expect(changeHappend).toBe(true);
});
